...
export default function HearingHistory({ caseId, refreshTrigger }: HearingHistoryProps) {
  ...
  const handleSubmit = async () => {
    if (!formData.date || !formData.time || !formData.type || !formData.judge || !formData.courtroom) {
    ...
    try {
      ...
      console.error(`Error ${editingHearing ? 'updating' : 'creating'} hearing:`, error);
      alert(error instanceof Error ? error.message : `Failed to ${editingHearing ? 'update' : 'create'} hearing`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteHearing = async (hearingId: string) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Failed to delete hearing');
    }
  };
  ...
  const handleStatusUpdate = async (hearingId: string, status: string, outcome?: string) => {
    try {
  ...
  const getOutcomeColor = (outcome: string | null) => {
    ...
    switch (outcome) {
      ...
      case 'Settlement Reached': return 'bg-green-100 text-green-800';
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-end space-x-4 pt-4">
              ...
              >
                {submitting ? (editingHearing ? 'Updating...' : 'Creating...') : (editingHearing ? 'Update Hearing' : 'Schedule Hearing')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div key={hearing.id} className="border border-gray-200 rounded-lg p-6">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
            ...
            {hearing.status === 'Scheduled' && (
              <div className="flex items-center space-x-3">
                <button 
                  onClick={() => handleStatusUpdate(hearing.id, 'Completed', 'Favorable')}
      ...
      {hearings.length === 0 && !loading && (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-500 mt-2">Click the "Schedule New Hearing" button above to get started</p>
        </div>
      )}
    </div>
  );
