...
export default function NotesSection({ caseId, refreshTrigger }: NotesSectionProps) {
  ...
  const handleAddNote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh notes list
      await fetchNotes();
      setNewNote({ content: '', type: 'Case Note', tags: '', priority: 'Medium' });
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'Client Communication': return 'bg-orange-100 text-orange-800';
      case 'Internal Note': return 'bg-indigo-100 text-indigo-800';
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'Client Communication': return 'ri-chat-3-line';
      case 'Internal Note': return 'ri-lock-line';
    ...
    <div>
      ...
      <div className="space-y-4">
        {filteredNotes.map((note) => (
          <div key={note._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition duration-200">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="text-right text-sm text-gray-500">
                ...
                <p>{formatDate(note.createdAt)} at {formatTime(note.createdAt)}</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full">
            <div className="p-6">
              ...
              <form onSubmit={handleAddNote} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    maxLength={1000}
                  />
                  <p className="text-xs text-gray-500 mt-1">{newNote.content.length}/1000 characters</p>
                </div>
              ...
              <div className="mt-4 p-2 bg-gray-50 rounded text-xs">
                <strong>Debug:</strong> Case ID: {caseId}, Notes count: {notes.length}
              </div>
            </div>
          </div>
        </div>
