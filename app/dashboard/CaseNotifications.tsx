    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-3 mb-6">
        {upcomingHearings.length > 0 ? (
          upcomingHearings.slice(0, 3).map((hearing: any) => {
            ...
            return (
              <Link key={hearing._id} href={`/cases/${hearing.caseId._id || hearing.caseId}`} className="cursor-pointer">
      ...
      {upcomingHearings.length > 3 && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <Link href="/hearings" className="text-blue-600 hover:text-blue-700 text-sm font-medium cursor-pointer">
            View All Hearings →
          </Link>
        </div>
      )}
